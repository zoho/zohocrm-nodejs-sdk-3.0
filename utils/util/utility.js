...
const ZCRMModule = require("../../core/com/zoho/crm/api/fields/module.js").Module
...
class Utility {
    ...
    static JSONDETAILS =  Initializer.jsonDetails;
    ...
    static async fileExistsFlow(moduleAPIName, recordFieldDetailsPath, lastModifiedTime) {
        ...
        if (moduleAPIName == null || (recordFieldDetailsJson.hasOwnProperty(moduleAPIName.toLowerCase()) && recordFieldDetailsJson[moduleAPIName.toLowerCase()] != null)) {
            ...
            fs.writeFileSync(recordFieldDetailsPath, JSON.stringify(recordFieldDetailsJson, (key, value) =>
            typeof value === 'bigint'
                ? value.toString()
        ...
        for (let index = 0; index < modulerelatedListArray.length; index++) {
            ...
            if (relatedListObject[Constants.API_NAME] != null && relatedListObject[Constants.API_NAME].toLowerCase() == relatedModuleName.toLowerCase()) {
                ...
                if (relatedListObject[Constants.MODULE].toLowerCase() != Constants.NULL_VALUE) {
                    ...
                    commonAPIHandler.setModuleAPIName(relatedListObject[Constants.MODULE].apiName);

                    await Utility.getFieldsInfo(relatedListObject[Constants.MODULE].apiName, commonAPIHandler);
                }
    ...
    static async writeModuleMetaData(recordFieldDetailsPath, moduleData) {
        ...
        fs.writeFileSync(recordFieldDetailsPath, JSON.stringify(fieldDetailsJSON, (key, value) =>
        typeof value === 'bigint'
            ? value.toString()
            : value // return everything else unchanged
    ));
    ...
    static async getModules(header) {
        ...
        if (header != null) {
            ...
            let  id = parseInt(header);

            await headerMap.add(GetModulesHeader.IF_MODIFIED_SINCE, new Date(id));
        }
